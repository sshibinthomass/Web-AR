...
export class WebARApp {
  ...
  private photoGenerationTransitionEpoch: number | null = null;
  ...
  private async runFullFlow(targetObject: string): Promise<void> {
    ...
    const mediaOperationEpoch = this.beginCapturedMediaOperation();
    this.hud?.clearObjectReconstruction();
    this.photoGenerationTransitionEpoch = mediaOperationEpoch;
    try {
      const capturedImage = this.capturedImage;
      ...
      await this.loadModelFromUrl(generatedModel.modelUrl, 'Generated object', {
        ...
        isCurrent: () => this.isCurrentCapturedMediaOperation(mediaOperationEpoch),
      });
      if (!this.isCurrentCapturedMediaOperation(mediaOperationEpoch)) {
        return;
      }
      ...
      if (this.photoGenerationTransitionEpoch === mediaOperationEpoch) {
        this.photoGenerationTransitionEpoch = null;
        if (this.isCurrentCapturedMediaOperation(mediaOperationEpoch)) {
          this.cancelObjectSegmentation();
        }
      }
  ...
  private async leaveRoute(previousRoute: HudRoute, nextRoute: HudRoute): Promise<void> {
    if (
      this.photoGenerationTransitionEpoch !== null
      && this.isCurrentCapturedMediaOperation(this.photoGenerationTransitionEpoch)
  ...
  private placeAtLatestHit(): void {
    ...
    if (this.layoutMode) {
      ...
      if (target) {
        ...
        const hitResult = hitTestManager.latestHitResult;
        if (hitResult) {
          void this.anchorManager?.createFor(target, hitResult);
        }
      }
    ...
    if (hitTestManager.latestHitResult) {
      void this.anchorManager?.createFor(this.requireScene().modelRoot, hitTestManager.latestHitResult);
    }
    this.appState.floorLocked = true;
    this.appState.setMode('placed');
    this.hud?.update(this.appState.mode);
